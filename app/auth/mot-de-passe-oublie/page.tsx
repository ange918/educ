...
export default function MotDePasseOubliePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Supabase ne révèle jamais si l'email existe ou non (anti-énumération de comptes) :
    // on affiche toujours le même message de confirmation, que l'email soit inscrit ou pas.
    setLoading(false)
    setEnvoye(true)
  }
    ...
    <div style={{ background: '#F7F5EF', minHeight: '100vh', display: 'flex' }}>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
        <div style={{ width: '100%', maxWidth: '420px' }}>
          ...
          {envoye ? (
            <>
              <div style={{ width: '64px', height: '64px', borderRadius: '16px', background: 'rgba(0,135,81,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '1.5rem' }}>
                <MailCheck size={30} color="#008751" />
              </div>
              <h2 style={{ fontFamily: 'Orbitron, sans-serif', fontWeight: 800, fontSize: '1.5rem', marginBottom: '0.75rem', color: '#14201A' }}>Vérifiez votre boîte mail</h2>
              <p style={{ color: '#6E7268', fontFamily: 'Inter, sans-serif', fontSize: '0.95rem', lineHeight: 1.7 }}>
                Si un compte existe avec l'adresse <strong style={{ color: '#14201A' }}>{email}</strong>, un lien de réinitialisation vient de lui être envoyé. Cliquez dessus pour choisir un nouveau mot de passe.
              </p>
              <p style={{ color: '#9AA093', fontFamily: 'Inter, sans-serif', fontSize: '0.85rem', marginTop: '1rem' }}>
                Vous ne voyez rien arriver ? Pensez à vérifier vos <strong>spams / courriers indésirables</strong>.
              </p>
            </>
          ) : (
            <>
              <h2 style={{ fontFamily: 'Orbitron, sans-serif', fontWeight: 800, fontSize: '1.8rem', marginBottom: '0.5rem', color: '#14201A' }}>Mot de passe oublié</h2>
              <p style={{ color: '#9AA093', fontFamily: 'Inter, sans-serif', fontSize: '0.9rem', marginBottom: '2rem' }}>Entrez votre email, on vous envoie un lien pour le réinitialiser.</p>
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                <div>
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    <input type="email" name="email" id="email" autoComplete="email" required style={inputStyle} value={email} onChange={e => setEmail(e.target.value)} placeholder="votre@email.com"
                ...
                <button type="submit" disabled={loading} style={{ background: loading ? '#E7E3D8' : 'linear-gradient(135deg, #008751, #00a862)', color: loading ? '#9AA093' : '#fff', padding: '1rem', borderRadius: '12px', fontFamily: 'Orbitron, sans-serif', fontWeight: 700, fontSize: '0.9rem', cursor: loading ? 'not-allowed' : 'pointer', border: 'none', marginTop: '0.5rem', transition: 'all 0.3s', boxShadow: loading ? 'none' : '0 8px 30px rgba(0,135,81,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                  {loading ? 'Envoi...' : <><Send size={16} /> Envoyer le lien</>}
                </button>
              </form>
            </>
          )}
