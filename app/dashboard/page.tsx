          ...
          <div style={{ display: 'flex', gap: '1rem', marginBottom: '3rem', flexWrap: 'wrap' }}>
            {[
              ...
              <Link key={label} href={href}>
                <button style={{ background: '#FFFFFF', border: `1px solid ${color}33`, color: color, padding: '0.75rem 1.25rem', borderRadius: '10px', fontFamily: 'Inter, sans-serif', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem', transition: 'all 0.2s' }}
                  ...
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = `${color}22`; (e.currentTarget as HTMLElement).style.background = '#FFFFFF' }}>
                  {icon} {label}
                </button>
              </Link>
            ))}
          ...
          {tenues.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {tenues.map(tenue => {
                ...
                return (
                  <div key={tenue.id} style={{ background: '#FFFFFF', border: '1px solid #E7E3D8', borderRadius: '12px', padding: '1.25rem', display: 'flex', gap: '1.25rem', alignItems: 'center', flexWrap: 'wrap', transition: 'border-color 0.2s' }}
                    onMouseEnter={e => (e.currentTarget.style.borderColor = '#E7E3D8')}
                    onMouseLeave={e => (e.currentTarget.style.borderColor = '#E7E3D8')}>
                    <div style={{ position: 'relative', width: '72px', height: '72px', flexShrink: 0, borderRadius: '10px', overflow: 'hidden', background: '#E7E3D8' }}>
                      {photo ? (
                        <Image src={photo} alt={tenue.nom} fill style={{ objectFit: 'cover' }} />
                      ) : (
                        <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <Shirt size={24} color="#C4C0B3" />
                        </div>
                      )}
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      ...
                      <button onClick={() => handleSupprimer(tenue.id)} style={{ background: 'rgba(232,17,45,0.1)', border: '1px solid rgba(232,17,45,0.2)', color: '#E8112D', padding: '0.5rem 1rem', borderRadius: '8px', fontFamily: 'Inter, sans-serif', fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
