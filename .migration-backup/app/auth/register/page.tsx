  ...
  const handleSubmit = async () => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem(
        'edusecure_session',
        JSON.stringify({ role: 'parent', name: `${form.firstName} ${form.lastName}`, email: form.email, token: 'mock-jwt-token' }),
      )
    }
    setLoading(false)
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center gap-4 text-center">
          ...
          <p className="text-slate-400 text-base max-w-xs leading-relaxed mt-2">
            Rejoignez ÉduSecure et gérez la scolarité de vos enfants simplement.
          </p>
        </div>
      </div>
        ...
        <div className="w-full max-w-sm animate-fade-in">
            ...
            {steps.map((s, i) => (
              <React.Fragment key={s.n}>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className={`text-xs ${step === s.n ? 'text-gray-700 font-medium' : 'text-gray-400'}`}>
                    {s.label}
                  </span>
                </div>
                {i < steps.length - 1 && (
                  <div className={`flex-1 h-px ${step > s.n ? 'bg-emerald-400' : 'bg-gray-200'}`} />
                )}
              </React.Fragment>
            ))}
          </div>
          ...
          {step === 1 && (
            <div className="space-y-4 animate-fade-in">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="Akua"
                      className="w-full pl-9 pr-3 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-accent"
                    />
                  </div>
                </div>
                <div>
                  ...
                  <input
                    ...
                    placeholder="Osei"
                    className="w-full px-3 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-accent"
                  />
                </div>
              </div>
              <div>
              ...
              <button
                onClick={() => setStep(2)}
                className="w-full bg-accent text-white py-3.5 rounded-xl font-semibold text-sm hover:bg-accent-light transition-colors active:scale-95 duration-100 mt-2"
              >
                Suivant →
              </button>
            </div>
            ...
            <div className="space-y-4 animate-fade-in">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="••••••••"
                    className="w-full pl-9 pr-10 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-accent"
                  />
                  <button
                    type="button"
              ...
              <div className="flex gap-3 mt-2">
                ...
                <button
                  ...
                  className="flex-1 bg-accent text-white py-3.5 rounded-xl font-semibold text-sm hover:bg-accent-light transition-colors active:scale-95 duration-100 disabled:opacity-60"
                >
                  Suivant →
                </button>
              </div>
            ...
            <div className="space-y-4 animate-fade-in">
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  disabled={!form.accepted || loading}
                  className="flex-1 bg-accent text-white py-3.5 rounded-xl font-semibold text-sm hover:bg-accent-light transition-colors active:scale-95 duration-100 disabled:opacity-60 flex items-center justify-center gap-2"
                >
                  {loading ? (
                    <>
