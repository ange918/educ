...
export default function StudentsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (activeTab !== 'Toutes') {
      list = list.filter((s) => s.classe.startsWith(activeTab))
    }
    if (search) {
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead className="bg-gray-50 border-b border-gray-200">
          <tr>
            {['#', 'Nom complet', 'Classe', 'N° Matricule', 'Montant payé', 'Reste à payer', 'Statut', 'Actions'].map(
              (h) => (
        ...
        <tbody className="divide-y divide-gray-100">
          {students.map((s, i) => (
            <tr
              key={s.id}
              className="hover:bg-accent-pale transition-colors cursor-pointer"
            ...
            >
              ...
              <td className="px-4 py-3.5">
                <span
                  className={`font-semibold ${
                    s.remaining === 0 ? 'text-emerald-600' : 'text-amber-600'
                  }`}
                >
                  {formatXOF(s.paid)}
                </span>
              ...
              <td className="px-4 py-3.5">
                {s.remaining > 0 ? (
                  <span className="text-danger font-semibold">{formatXOF(s.remaining)}</span>
                ) : (
              ...
              <td className="px-4 py-3.5">
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className="p-1.5 text-gray-400 hover:text-accent hover:bg-accent-pale rounded-lg transition-colors"
                  >
                    <i className="bx bx-eye text-base" />
                  </button>
                  <button
      ...
      <div className="animate-fade-in">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">
              {mockStudents.length} élèves inscrits · {activeTab !== 'Toutes' ? filtered.length + ' dans cette classe' : ''}
            </p>
          </div>
            ...
            Object.entries(groupedByClass).map(([classe, students]) => (
              <div key={classe}>
                <div className="px-5 py-3.5 bg-gray-50 border-b border-gray-200 flex items-center gap-3">
                  ...
                  <span className="bg-accent-pale text-accent text-xs px-2.5 py-0.5 rounded-full font-semibold">
        ...
        {filtered.length === 0 && (
          <div className="text-center py-16 text-gray-400">
            <i className="bx bx-search text-4xl mb-3 block" />
          ...
          <div className="fixed right-0 top-0 bottom-0 z-50 w-full md:w-[420px] bg-white shadow-2xl animate-slide-in-right overflow-y-auto">
            <div className="p-6">
              ...
              <div className="flex flex-col items-center text-center mb-6">
                <div
                  className="w-20 h-20 rounded-full flex items-center justify-center text-white font-display font-bold text-2xl mb-3"
                  style={{ backgroundColor: getAvatarColor(`${selectedStudent.firstName} ${selectedStudent.lastName}`) }}
                >
                ...
                <div className="flex items-center gap-2 mt-2 flex-wrap justify-center">
                  <span className="bg-accent-pale text-accent text-xs px-2.5 py-1 rounded-full font-medium">
                    {selectedStudent.classe}
                  </span>
                  <span className="font-mono text-xs text-gray-400">{selectedStudent.numero}</span>
                </div>
                <div className="mt-2">
                  <StatusBadge status={selectedStudent.accessStatus} />
                </div>
              </div>
              ...
              <div className="bg-gray-50 rounded-2xl p-5 mb-5">
                ...
                <div className="space-y-2 mb-3">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-semibold text-gray-800">{formatXOF(selectedStudent.totalFees)}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">Payé</span>
                    <span className="font-semibold text-emerald-600">{formatXOF(selectedStudent.paid)}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">Reste</span>
                    <span className={`font-semibold ${selectedStudent.remaining > 0 ? 'text-danger' : 'text-gray-400'}`}>
                      {selectedStudent.remaining > 0 ? formatXOF(selectedStudent.remaining) : '—'}
                    </span>
                  </div>
                </div>
                <div className="h-2 bg-white rounded-full overflow-hidden border border-gray-200">
                  <div
                    ...
                    style={{
                      width: `${Math.round((selectedStudent.paid / selectedStudent.totalFees) * 100)}%`,
                    }}
                  />
                </div>
                <div className="text-xs text-gray-400 mt-1">
                  {Math.round((selectedStudent.paid / selectedStudent.totalFees) * 100)}% payé
                </div>
                {selectedStudent.lastPaymentDate && (
                  <div className="text-xs text-gray-400 mt-2">
                    Dernier paiement le{' '}
                    {new Date(selectedStudent.lastPaymentDate).toLocaleDateString('fr-FR', {
                      day: 'numeric',
                      month: 'long',
                      year: 'numeric',
                    })}
              ...
              <div className="space-y-3">
                <button className="w-full border border-accent text-accent py-3 rounded-xl text-sm font-semibold hover:bg-accent-pale transition-colors flex items-center justify-center gap-2">
                  <i className="bx bx-edit" />
                  Modifier le statut
                </button>
                <button className="w-full bg-accent text-white py-3 rounded-xl text-sm font-semibold hover:bg-accent-light transition-colors flex items-center justify-center gap-2">
