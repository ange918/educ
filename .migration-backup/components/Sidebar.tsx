...
export default function Sidebar({ role, userName = 'Akua Osei' }: SidebarProps) {
  ...
  const sidebarBg = role === 'admin' ? 'bg-midnight' : 'bg-midnight-mid'
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside
        className={`hidden md:flex flex-col w-60 min-h-screen ${sidebarBg} fixed left-0 top-0 z-30`}
      >
          ...
          {nav.map((item) => {
            ...
            return (
              ...
              >
                ...
                {item.badge && (
                  <span className="bg-danger text-white text-xs w-5 h-5 rounded-full flex items-center justify-center font-bold">
                    {item.badge}
                  </span>
                )}
              </Link>
      ...
      <nav
        className={`md:hidden fixed bottom-0 left-0 right-0 z-30 ${sidebarBg} border-t border-white/10 flex`}
        style={{ height: 60 }}
      >
        {nav.map((item) => {
          const isActive = pathname === item.href
        ...
        <button
          onClick={handleLogout}
          className="flex-1 flex flex-col items-center justify-center gap-0.5 text-xs text-slate-400"
        >
          <i className="bx bx-log-out text-xl" />
